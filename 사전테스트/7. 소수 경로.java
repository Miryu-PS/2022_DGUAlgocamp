...
public class main {
	...
	public static void main(String[] args) throws IOException {
		...
		isPrime = new int[10001]; // isPrime[x] = x가 소수면 0, 아니면 1
	...
	public static ArrayList<Integer> getlist(int x) { // x에서 한 자리만 바꿔서 만들 수 있는 소수들의 목록
		ArrayList<Integer> arr = new ArrayList<Integer>();
		for(int i=1 ; i<10; i++) { //천의 자리
			int tmp = i*1000 + (x%1000);
			if(isPrime[tmp]==0 && x!=tmp) arr.add(tmp);
		}
		for(int i=0; i<10; i++) { //백의 자리
			int tmp = (x/1000)*1000 + i*100 + (x%100);
			if(isPrime[tmp]==0 && x!=tmp) arr.add(tmp);
		}
		for(int i=0; i<10; i++) { //십의 자리
			int tmp = (x/100)*100 + i*10 + (x%10);
			if(isPrime[tmp]==0 && x!=tmp) arr.add(tmp);
		}
		for(int i=0; i<10; i++) { //일의 자리
			int tmp = (x/10)*10 + i;
			if(isPrime[tmp]==0 && x!=tmp) arr.add(tmp);
		}
		return arr;
